	...
	function function_delete(){
		...
		for (var i=0; i<obj_length; i++) {
            if (document.getElementsByName("custno")[i].checked == true) {
            	...
            	if( returnValue == true){
            		location.href="DeleteMember.jsp?custno="+document.getElementsByName("custno")[i].value+"&action=delete";	
            	}else{
            		return;	
            	}
            }
	...
	<div class="container">
		<div class="content">
			...
			<form action="SelectMember.jsp">
				...
				<input type="button" onClick="location.href='NewMemberForm.jsp'" value="등록">
				<input type="button" onClick="function_delete()" value="삭제">
				<input type="button" onClick="function_update()" value="수정">
				<input type="button" onClick="location.href='action.jsp?action=stack'" value="스택"> 
				<input type="button" style="width:23pt;" onClick="location.href='action.jsp?action=queue'" value="큐">
			</form>
			...
			if (name != null && name != "") {
				String query2 = "select mn.custno, mb.custname, mn.price from member_tbl_02 mb, money_tbl_02 mn	where mb.custno = mn.custno and mb.custname='"
								+ name + "'";	
				ResultSet result2 = conn.prepareStatement(query2).executeQuery();
				if (result2.next()) {
					do {
